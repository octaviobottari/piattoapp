...
def configurar_restaurante(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if logo_file:
                print(f"Subiendo logo: {logo_file.name} para {restaurante.username}")  # Depuración
            try:
                form.save()
                print(f"Logo guardado para {restaurante.username}. Ruta en S3: {restaurante.logo.url if restaurante.logo else 'No logo'}")  # Depuración
            except Exception as e:
                print(f"Error al guardar el formulario: {str(e)}")  # Depuración
...
def metricas_view(request):
    ...
    # Fetch orders within date range with valid states
    pedidos = Pedido.objects.filter(
        restaurante=restaurante,
        fecha__date__gte=fecha_inicio,
        fecha__date__lte=fecha_fin,
        estado__in=['pagado', 'en_preparacion', 'listo', 'entregado', 'archivado', 'en_entrega']
    ).prefetch_related('items__producto')

    # Check if all products have costo_produccion
    productos = Producto.objects.filter(restaurante=restaurante)
    costos_completos = productos.exists() and all(p.costo_produccion is not None for p in productos)

    # Calculate total profit using aggregation
    ganancia_total = Decimal('0.00')
    if costos_completos:
        items = ItemPedido.objects.filter(
            pedido__in=pedidos,
            producto__costo_produccion__isnull=False
        ).select_related('producto')
        for item in items:
            if item.producto and item.producto.costo_produccion is not None:
                ganancia_total += (item.precio_unitario - item.producto.costo_produccion) * item.cantidad

    # Calculate profit percentage change for previous period
    previous_period_start = fecha_inicio - (fecha_fin - fecha_inicio)
    previous_pedidos = Pedido.objects.filter(
        restaurante=restaurante,
        fecha__date__gte=previous_period_start,
        fecha__date__lt=fecha_inicio,
        estado__in=['pagado', 'en_preparacion', 'listo', 'entregado', 'archivado', 'en_entrega']
    )
    ganancia_anterior = Decimal('0.00')
    if costos_completos:
        previous_items = ItemPedido.objects.filter(
            pedido__in=previous_pedidos,
            producto__costo_produccion__isnull=False
        ).select_related('producto')
        for item in previous_items:
            if item.producto and item.producto.costo_produccion is not None:
                ganancia_anterior += (item.precio_unitario - item.producto.costo_produccion) * item.cantidad
    ganancia_porcentaje_cambio = 0.0
    if ganancia_anterior > 0:
        ganancia_porcentaje_cambio = float(((ganancia_total - ganancia_anterior) / ganancia_anterior) * 100)

    # Placeholder profit line graph path (replace with dynamic data if needed)
    profit_line_path = "M10 60 Q40 20 70 40 T130 30 T190 50 T230 40"
    ...
    for month in range(1, 13):
        month_start = datetime(fecha_inicio.year, month, 1).date()
        if month == 12:
            month_end = datetime(fecha_inicio.year + 1, 1, 1).date() - timedelta(days=1)
        else:
            month_end = datetime(fecha_inicio.year, month + 1, 1).date() - timedelta(days=1)
        if month_end < fecha_inicio or month_start > fecha_fin:
            ventas_por_mes.append({'mes': calendar.month_abbr[month], 'cantidad': 0, 'altura': 0})
            total_por_mes.append(Decimal('0.00'))
            continue
        month_pedidos = pedidos.filter(fecha__date__gte=month_start, fecha__date__lte=month_end)
        cantidad = month_pedidos.count()
        total = month_pedidos.aggregate(total=Sum('total'))['total'] or Decimal('0.00')
        ventas_por_mes.append({'mes': calendar.month_abbr[month], 'cantidad': cantidad, 'altura': 0})
        total_por_mes.append(total)
        max_ventas = max(max_ventas, cantidad)
        max_total = max(max_total, total)

    # Normalize bar heights (max height 140px)
    for venta in ventas_por_mes:
        if max_ventas > 0:
            venta['altura'] = (venta['cantidad'] / max_ventas) * 140

    # Y-axis labels for sales chart
    y_labels = [800, 600, 400, 200, 0]
    if max_ventas > 0:
        y_labels = [int(max_ventas * i / 4) for i in range(4, -1, -1)]

    # Sales line graph points
    sales_line_points_data = []
    if max_total > 0:
        for i, total in enumerate(total_por_mes):
            x = 20 + i * (550 / 11)
            y = 140 - (float(total) / float(max_total)) * 140
            sales_line_points_data.append({'x': x, 'y': y})
    else:
        sales_line_points_data = [{'x': 20 + i * (550 / 11), 'y': 140} for i in range(12)]
    sales_line_points = ' '.join([f"{point['x']},{point['y']}" for point in sales_line_points_data])

    # Sales by day of week
    ventas_por_dia = [
        {'dia': 'Domingo', 'cantidad': 0, 'altura': 0},
        {'dia': 'Lunes', 'cantidad': 0, 'altura': 0},
        {'dia': 'Martes', 'cantidad': 0, 'altura': 0},
        {'dia': 'Miércoles', 'cantidad': 0, 'altura': 0},
        {'dia': 'Jueves', 'cantidad': 0, 'altura': 0},
        {'dia': 'Viernes', 'cantidad': 0, 'altura': 0},
        {'dia': 'Sábado', 'cantidad': 0, 'altura': 0},
    ]
    max_ventas_dia = 0
    for i, dia in enumerate(ventas_por_dia):
        # Django's week_day: 1=Sunday, 2=Monday, ..., 7=Saturday
        cantidad = pedidos.filter(fecha__week_day=(i % 7) + 1).count()
        ventas_por_dia[i]['cantidad'] = cantidad
        max_ventas_dia = max(max_ventas_dia, cantidad)
    for dia in ventas_por_dia:
        if max_ventas_dia > 0:
            dia['altura'] = (dia['cantidad'] / max_ventas_dia) * 100
    ...
    dia_actual = {
        ...
        'Friday': 'Viernes',
        'Saturday': 'Sábado',
    }.get(dia_actual, 'Jueves')

    # Total products sold
    total_productos = ItemPedido.objects.filter(pedido__in=pedidos).aggregate(total=Sum('cantidad'))['total'] or 0

    # Last 5 transactions
    ultimos_pedidos = ItemPedido.objects.filter(pedido__in=pedidos).select_related('pedido', 'producto').order_by('-pedido__fecha')[:5]

    context = {
        'costos_completos': costos_completos,
        'ganancia_total': ganancia_total,
        'ganancia_porcentaje_cambio': ganancia_porcentaje_cambio,
        'profit_line_path': profit_line_path,
        'ventas_por_mes': ventas_por_mes,
        'y_labels': y_labels,
        'sales_line_points': sales_line_points,
        'sales_line_points_data': sales_line_points_data,
